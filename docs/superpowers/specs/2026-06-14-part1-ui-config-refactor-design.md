...
```jsonc
{
  "mapping": "openrouter:gpt-5.5",   // 全局单一 mapping;"pass" 表示透传
  "providers": [
    // preset 条目:极简,只有 name + apiKeys
    { "name": "glm", "apiKeys": ["sk-a", "sk-b"] },

    // 自定义 openai-compatible 条目:需自带 baseUrl 与 modelsFrom
    {
      "name": "my-endpoint",
      "format": "openai",
      "baseUrl": "https://my.host/v1",
      "apiKeys": ["sk-x"],
      "modelsFrom": "openai",          // 模型列表借用 models.dev 的某 provider;可缺省为 openai
      "models": ["gpt-4o"]             // 可选:手动覆盖,留空则用 modelsFrom 的列表
    }
  ]
}
```
...
- **preset 条目极简**:只存 `name`(= preset id,见下表)+ `apiKeys`;`format` / `baseUrl` 来自内置 preset 目录,`models` 来自 models.dev。需要时可在条目里补 `baseUrl` 等字段手动覆盖。
- **自定义条目**:`format` 固定 `openai`,自带 `baseUrl`;模型列表通过 `modelsFrom`(指向某 models.dev id,默认 `openai`)获取,或用 `models` 手动覆盖。
...
| openai-compatible | openai | 用户自填 | 由 modelsFrom 决定 | ❌(Part 2) |
...
- preset 通过表中的 models.dev id 取列表;自定义 provider 通过 `modelsFrom` 取;两者均可用 `models` 字段手动覆盖。
...
- 进入后弹出命令面板式 QuickPick,**列出全部 preset + "添加 OpenAI 兼容…"**(此处才包含尚未配置 key 的 provider)。
- 选中某 provider → **管理 key**:列出已有 key(掩码显示)、`添加 key`(输入框)、选中某 key 删除。
- 选 "添加 OpenAI 兼容…" → 依次输入 name、baseUrl、(可选)modelsFrom、key,写入一条自定义 provider。
...
3. `⚙ Provider 设置` 可列出全部 preset + "添加 OpenAI 兼容…",并对选中 provider 增 / 删 key,改动写回 `providers.json`;key 归零后该 provider 从 `[Provider]` 段消失。
4. 模型列表来自 models.dev 并本地缓存;preset 按映射 id 取、自定义按 `modelsFrom` 取;离线时用缓存。
