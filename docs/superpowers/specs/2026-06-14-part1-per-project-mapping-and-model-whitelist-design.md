...
匹配规则:`*` 匹配任意长度任意字符、`.` 匹配任意单个字符(因此 `kimi-k2.*` 能命中 `kimi-k2-0905` 这类连字符 id)、大小写不敏感。匹配只针对 model id 中 `/` 之后的**模型名**部分,忽略 vendor 前缀(nvidia/openrouter 等聚合平台的 id 形如 `z-ai/glm-5.1`、`moonshotai/kimi-k2.6`,需匹配 `glm-5.1`/`kimi-k2.6`)。命中的**全部显示、不做数量截断**;未命中的收进 `$(ellipsis) Other…`(展开后显示全部)。
